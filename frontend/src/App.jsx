...
const KEYWORDS = [
  ...
  "amenity misuse", "pool party", "after hours", "after-hours", "noise",
  "loud", "non-residents", "illegal parking", "trash dumping", "package theft"
];
...
export default function App() {
  ...
  const [proxyBase, setProxyBase] = useState("https://vidisky-reviews.onrender.com"); // ← set once deployed
  ...
  async function onFetch() {
    try {
      ...
      // Call Google + ApartmentRatings (skip Apartments.com)
  ...
  function copyEmail() {
    ...
    lines.push("");
    lines.push("Open to a 15-minute walkthrough to quantify impact (incl. pet-waste fines & amenity enforcement)?");
    lines.push("– Stan");
  ...
  async function exportPdf() {
    ...
    if (ih < ph - 40) {
      ...
      let left = ih, pos = 20;
      pdf.addImage(img, "PNG", 20, pos, iw, ih);
      left -= ph - 40;
      while (left > 0) {
        pdf.addPage();
        pos = 20 - (ih - left);
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", color: "#0f172a" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button onClick={onFetch} disabled={loading} style={{ background: "black", color: "white", borderRadius: 10, padding: "8px 12px" }}>
            {loading ? "Fetching…" : "Fetch & Summarize"}
          </button>
          <button onClick={copyEmail} style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: "8px 12px" }}>
            Copy Email
          </button>
          <button onClick={exportPdf} style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: "8px 12px" }}>
            Export PDF
          </button>
        </div>
              ...
              <ul style={{ margin: "8px 0 0 18px" }}>
                {filtered.slice(0, 30).map((q, i) => (
                  <li key={i} style={{ marginBottom: 6, fontSize: 14, lineHeight: "20px" }}>
                    “{q.sentence}” {q.url && <a href={q.url} target="_blank" rel="noreferrer" style={{ color: "#64748b", textDecoration: "underline", marginLeft: 6 }}>source</a>} <span style={{ color: "#94a3b8", fontSize: 12 }}>[{q.source}]</span>
                  </li>
                ))}
              </ul>
            )}
